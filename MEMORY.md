...
- Long blocking or obviously interactive `bash run` commands can auto-detach into a conversation Terminal after `SOCRATES_TERMINAL_AUTO_DETACH_MS` (default 60 seconds). The UI/product copy says Terminal while the model-visible tool id remains `bash`.
- Terminal input is user-only. The frontend sends raw xterm data through `terminal.input`; the backend never persists raw stdin, only redacted input markers. Conservative prompt detection can still mark a Terminal `awaiting_input` and emit `terminal.input.requested`.
...
- The chat workspace now has an xterm-backed persistent Terminal panel hydrated from `GET /api/projects/:projectId/conversations/:conversationId.terminals` and updated by `terminal.started`, `terminal.data`, `terminal.status`, `terminal.input.requested`, `terminal.completed`, `terminal.stopped`, and compatibility stale/detached/missing state changes. `terminal.resize` keeps backend PTY dimensions aligned with the panel.
