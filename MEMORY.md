...
- Local release tooling is validated with `pnpm@11.7.0`. Runtime archive packaging uses `pnpm deploy --legacy` for the server sidecar and commits a narrow `allowBuilds` list for required native runtime dependencies (`better-sqlite3`, `@homebridge/node-pty-prebuilt-multiarch`), required dev packaging toolchain packages (`esbuild`, `unrs-resolver`), and dependency setup packages (`msw`, `sharp`). GitHub Actions runs pnpm 11 under Node 24, while the produced runtime archive still bundles Node v20.20.2.
