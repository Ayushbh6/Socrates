...
- The `v0.1.20` release is deliberately **halted** pending investigation and correction of the reported workspace-tool reliability/latency issue (especially `edit`, plus Terminal and other action tools). The release-preparation commit `be29e36` remains on `main`, but the local and remote `v0.1.20` tags were deleted, GitHub Actions run `29873514078` was cancelled before the publish job, no `v0.1.20` GitHub Release exists, and npm `latest` remains `@socrates-ai/cli@0.1.19`. Do not recreate/tag/publish `v0.1.20` until the defect is fixed and the complete product-path verification succeeds.
