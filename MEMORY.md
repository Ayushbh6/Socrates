...
V2 Seamless Flow remains preserved redesign work, but it is not part of the stable production runtime. The stable line mounts only Classic routes/UI, performs no normal V2 reads or writes, and removes the packaged launcher flag. On first startup after upgrade, completed bridged Flow Q&A is recovered idempotently into Classic and all bridge ownership is returned to Classic; historical `v2_*` data is retained for a future deliberate V2 migration. The design authority remains `context-files/V2_FLOW_ARCHITECTURE.md` and must stay separate from stable repairs.
...
- Stable source and packaged servers do not mount `/api/v2/*` or `/v2/ws`, and the web build has no `/seamless` route or Flow switch. Do not re-enable these surfaces in a stable repair.
