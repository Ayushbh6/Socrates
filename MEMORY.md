...
- For generated deliverables or scratch files derived from files in a subfolder, Socrates should use explicit paths in that subfolder or the nearest relevant existing folder. The repo root is only the default for user-requested root output, truly project-level artifacts, or standalone workspace-level work with no relevant subfolder.
- `bash` is the stable model-visible compatibility id for the Terminal tool. User-facing and prompt copy should say Terminal. Terminal v3 uses a PTY backend with POSIX shells on macOS/Linux, Windows ConPTY through `powershell.exe`/`pwsh`/`cmd.exe`, and no separate model-visible PowerShell/cmd/process tools unless the contracts change.
- `bash operation: "run"` executes through a fresh PTY command, returns a lightly normalized terminal transcript plus exit/status metadata, streams bounded output, and enforces timeout/output caps. It no longer preserves exported environment or cwd between separate `run` calls; use one command or a started Terminal for durable interactive state.
- `bash` supports conversation-scoped PTY Terminal sessions with `operation: "start"`, then `status`/`output`/`stop` by no target when exactly one active Terminal exists or by human Terminal name when needed. Terminal/process ids remain internal for UI/runtime compatibility. Runtime ownership lives in the local Terminal supervisor, while SQLite persists metadata and ordered PTY output chunks for bounded replay.
- Long blocking or obviously interactive `bash run` commands can auto-detach into a conversation Terminal after `SOCRATES_TERMINAL_AUTO_DETACH_MS` (default 15 seconds). The UI/product copy says Terminal while the model-visible tool id remains `bash`.
- Terminal input is user-only. The frontend sends raw xterm data through `terminal.input`; the backend never persists raw stdin, only redacted input markers. Conservative prompt detection can still mark a Terminal `awaiting_input` and emit `terminal.input.requested`. `awaiting_input` is a hard human-handoff point: Socrates must tell the user what prompt is waiting, stop the response, and not declare interactivity successful until user input plus follow-up output are observed. Model-authored `stop` is rejected while a Terminal is awaiting user input; the user can still cancel from the Terminal shell.
...
- For subfolder command execution, Socrates should pass `bash.cwd` instead of prefixing commands with `cd`; before Terminal commands create files or directories, it should verify the intended parent directory and use explicit relative paths or `cwd`.
...
- The chat workspace now has an xterm-backed Terminal shell hydrated from `GET /api/projects/:projectId/conversations/:conversationId.terminals` and updated by `terminal.started`, `terminal.data`, `terminal.status`, `terminal.input.requested`, `terminal.completed`, `terminal.stopped`, and compatibility stale/detached/missing state changes. Desktop uses a compact right rail for overview/history plus a bottom dock for the focused terminal; mobile uses a full-screen terminal sheet. `terminal.resize` keeps backend PTY dimensions aligned with the active xterm surface.
...
- The code-generation path guidance is non-semantic and prompt/tool-contract based: it does not guess user intent from words, but it tells the model to keep generated outputs near the files it inspected when that relationship is already evident from tool use.
...
- The frontend chat workspace has a persistent Terminal shell with a compact right rail for status/history and a focused dock or mobile sheet for the active xterm surface. It exposes status, command/cwd metadata, bounded live output, stop controls, and user-only stdin controls when a Terminal awaits input. Timeline rows still show compact tool provenance and label `bash` as Terminal in UI copy.
...
- Terminal stdin is usable for running Terminals, not only perfectly detected awaiting-input rows. The UI is xterm-backed and supports raw keyboard input, paste, Ctrl-C, arrow navigation, resizing, copy/clear, and bounded replay after refresh.
...
- OpenRouter model routing is now cost-aware for the whole catalog. Multi-provider models no longer go out with empty routing; they default to price-first routing, DeepSeek V4 routes use cheap-compatible ranked provider orders, deliberate pins use OpenRouter provider slugs, and unknown OpenRouter models also fall back to price-first routing instead of OpenRouter's default load balancing. The stable OpenRouter cache-affinity key is sent as top-level `session_id` and `prompt_cache_key`, not nested under `extraBody`. During one agent turn, later OpenRouter continuations prefer the first actual routed provider reported by usage metadata while keeping fallbacks allowed.
- OpenAI prompt caching remains provider-automatic; Socrates sends `providerOptions.openai.promptCacheKey` for stable cache-affinity. Google/Gemini implicit caching remains provider-automatic; Socrates does not create explicit Gemini cached-content resources by default.
- Usage rows now persist `routed_provider` so audits can see which upstream endpoint actually billed a request. For direct providers this records the provider id itself; historical rows from before the schema change can be blank.
- A shared turn-efficiency eval now exists for the Socrates-vs-OpenCode cost work. It flags too many model round-trips, weak prompt-cache reuse, missing routed-provider capture, blocked routed providers, and high blended prompt cost. The current thresholds are tuned around OpenRouter-heavy turns and should not be treated as a universal provider-agnostic pricing oracle.
- The chat header shows cumulative conversation cost beside the context token count, with subtle markers for computed or unknown costs.
...
- Terminal model-facing use stays human-handle based. `bash` status/output/stop can omit the target when exactly one active Terminal exists or use the human Terminal name; terminal ids, process ids, PTY chunk sequences, and resize details are internal persistence/UI/supervisor details.
- Long-running Terminals are owned by a durable local PTY supervisor process. Server restart reconciliation marks uncontrollable persisted rows as `detached` or `missing`; `stale` is legacy/backward-compatible status language, not the preferred user/model-facing state.
- Terminal output requests drain supervisor output first, persist new PTY chunks, and return recent DB-backed output by human handle so background polling does not make model-facing output blank.
