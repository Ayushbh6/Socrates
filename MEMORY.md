...
- Added final release packaging flow: SemVer tags build signed macOS arm64 DMG and signed Windows x64 NSIS artifacts through GitHub Actions, publish updater artifacts/latest.json/SHA256SUMS/install scripts to GitHub Releases, and keep package-manager distribution deferred. Packaged users store provider credentials in OS keychain through Tauri commands; OpenRouter is required for chat/compression, OpenAI is required only for hosted embeddings when local Ollama is not used, and Google is optional. Credential APIs expose only status/source and must never return raw key values.
