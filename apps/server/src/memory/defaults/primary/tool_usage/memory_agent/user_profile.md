...
- `evidence_index`: traceable source anchors for important profile claims. It should record where important profile facts came from: date, project title/id, conversation title/id, turn/message/event ids when available, the supported claim, and which profile section uses that claim.
...
Use exact turn/message/event ids when they make the source retrievable. If ids are unavailable, use the best project title, conversation title, date, and source description. Do not put vague summaries or routine turns here.
<!-- /socrates:section -->
