...
- For an interactive user test, use `operation: "start"`, `inputMode: "user"`, a clear name, and one portable program that stays alive through the full exchange. The command must be complete and executable, such as `python3 -c ...` or `node -e ...`; never pass bare source code as the shell command. `inputMode` is the reliable orchestration signal; prompt wording is not. Prefer a small Node.js or Python stdin program. Do not use Bash-specific `read -p` on POSIX: the reported shell may be zsh, where it has different semantics.
