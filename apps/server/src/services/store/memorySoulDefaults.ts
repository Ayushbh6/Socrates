...
export const ensureStructuredSoulFile = (filePath: string, document: "identity" | "operating_principles"): void => {
  const content = document === "identity" ? DEFAULT_IDENTITY_MARKDOWN : DEFAULT_OPERATING_PRINCIPLES_MARKDOWN
  const legacySeeds = document === "identity" ? [LEGACY_IDENTITY_SEED] : [LEGACY_OPERATING_PRINCIPLES_SEED]
