...
class OllamaAdapter(BaseProvider):
    ...
    def _structured_output_query(self, response_model: Any) -> str:
        schema_json = json.dumps(
            self._normalize_schema(response_model.model_json_schema()),
            separators=(",", ":"),
            ensure_ascii=True,
        )
        return (
    ...
    def _execute_tool_call_sync(self, tool_call: ToolCall) -> str:
        ...
        if inspect.isawaitable(result):
            raise ValueError("Async tool executors are not supported by sync Ollama methods")
    ...
    def _create_tool_phase_request(
        self,
        request: LLMRequest,
        history: List[Message],
        query: str,
    ...
    def _create_final_phase_request(self, request: LLMRequest, history: List[Message]) -> LLMRequest:
        ...
        return LLMRequest(
            system_prompt=request.system_prompt,
            query=self._structured_output_query(request.response_model),
            history=history,
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        for _ in range(self._MAX_TOOL_ROUNDS):
            phase_request = self._create_tool_phase_request(
                request,
                history,
                active_query,
                attachments=request.attachments if not initial_recorded and active_query == request.query else None,
            )
    ...
    async def astream(self, request: LLMRequest) -> AsyncGenerator[LLMResponse, None]:
        if not self._should_use_two_phase(request):
            stream = self._astream_once(request, include_response_format=True, include_tools=True)
            try:
        ...
        for _ in range(self._MAX_TOOL_ROUNDS):
            ...
            phase_stream = self._astream_once(
                phase_request,
                include_response_format=False,
                include_tools=True,
            )
            try:
                async for response in phase_stream:
                    if response.content:
                        content_parts.append(response.content)
                    if response.thinking:
                        thinking_parts.append(response.thinking)
