...
def seed_blocked_worker_run(task: dict) -> str:
    session = get_session_factory()()
    try:
        run = AgentRun(
            project_id=task["project_id"],
            ...
            input_mode="text",
            system_prompt_text="system",
            query_text="seed blocked worker",
            request_json={},
            final_parsed_json={
                "status": "blocked",
                "summary": "Worker blocked during recovery test.",
                "blockers": [{"type": "todo_incomplete", "remaining_ids": ["T1"]}],
            },
            error_message="Worker blocked during recovery test.",
        )
        session.add(run)
        session.commit()
        run_id = run.id
...
def recovery_provider(content: str = "Recovery run handled.") -> FakeProvider:
    return FakeProvider(
        [
            {
                "chunks": [
...
@pytest.mark.parametrize(
    ("action_id", "seed_status"),
    [
        ("retry_remaining_work", "cancelled"),
        ("revise_plan", "stalled"),
    ],
)
def test_recovery_action_creates_socrates_run_for_recoverable_task(
    client: TestClient, monkeypatch, action_id: str, seed_status: str
):
    ...
    seed_task_run_status(task, status=seed_status)

    response = client.post(
        f"/api/v1/tasks/{task['id']}/recovery-actions",
        json={"action_id": action_id, "note": "please recover"},
    )
    ...
    events = drain_run_events(client, payload["agent_run_id"])
    assert events[-1]["type"] == "run.completed"
    assert provider.requests
    assert f"Requested action: {action_id}" in provider.requests[0].query
    assert "please recover" in provider.requests[0].query

    messages = client.get(f"/api/v1/conversations/{conversation_id}/messages").json()
    generated = [message for message in messages if message["metadata"].get("kind") == "task_recovery_action"]
    assert generated[-1]["metadata"]["action_id"] == action_id
    assert generated[-1]["agent_run_id"] == payload["agent_run_id"]


def test_recovery_action_accept_partial_output_starts_verification_without_closing_task(
    client: TestClient, monkeypatch
):
    provider = recovery_provider("Partial output verification handled.")
    _, _, task = prepare_task_with_todo(
        client,
        monkeypatch,
        todo_content=STANDARD_VALID_TODO,
        extra_providers=[provider],
    ...
    response = client.post(
        f"/api/v1/tasks/{task['id']}/recovery-actions",
        json={"action_id": "accept_partial_output"},
    )
    assert response.status_code == 202
    payload = response.json()
    drain_run_events(client, payload["agent_run_id"])
    assert provider.requests
    assert "partial output" in provider.requests[0].query.lower()
    active_task = client.get(f"/api/v1/tasks/{task['id']}").json()
    assert active_task["status"] == "active"


def test_recovery_action_retry_from_worker_blocked_starts_socrates_run(
    client: TestClient, monkeypatch
):
    provider = recovery_provider()
    _, _, task = prepare_task_with_todo(
    ...
    seed_blocked_worker_run(task)

    response = client.post(
        f"/api/v1/tasks/{task['id']}/recovery-actions",
        json={"action_id": "retry_remaining_work"},
    )
    assert response.status_code == 202
    payload = response.json()
    drain_run_events(client, payload["agent_run_id"])
    assert provider.requests
    assert "Recovery kind: worker_blocked" in provider.requests[0].query


def test_recovery_action_close_failed_uses_strict_closure_and_preserves_artifacts(
    client: TestClient, monkeypatch
):
    _, _, task = prepare_task_with_todo(
        client,
    ...
    seed_task_run_status(task, status="cancelled")
    artifacts_before = client.get(f"/api/v1/tasks/{task['id']}/artifacts").json()
    assert artifacts_before

    response = client.post(
        f"/api/v1/tasks/{task['id']}/recovery-actions",
        json={
            "action_id": "close_task_failed",
            "reason": "User chose to abandon this recoverable task.",
        },
    )
    assert response.status_code == 202
    payload = response.json()
    ...
    assert payload["task"]["result_summary"] == "User chose to abandon this recoverable task."

    artifacts_after = client.get(f"/api/v1/tasks/{task['id']}/artifacts").json()
    assert [artifact["id"] for artifact in artifacts_after] == [artifact["id"] for artifact in artifacts_before]


@pytest.mark.parametrize("action_id", ["unknown_action", "start_separate_task"])
def test_recovery_action_rejects_unknown_or_deferred_actions(
    client: TestClient, monkeypatch, action_id: str
):
    _, _, task = prepare_task_with_todo(
        client,
        monkeypatch,
    ...
    assert detail["code"] in {"unknown_recovery_action", "recovery_action_not_implemented"}


def test_recovery_action_rejects_action_not_allowed_for_current_state(
    client: TestClient, monkeypatch
):
    _, _, task = prepare_task_with_todo(
        client,
    ...
    assert response.json()["detail"]["code"] == "recovery_action_not_allowed"
