...
Current runtime release target is GitHub Release `v0.1.18`. It includes the v0.1.17 provider, retrieval, model-catalog, Ollama, and runtime-packaging foundation plus semantic user-controlled memory opt-outs, complete persisted tool-call traces with distinct intent-discovery UI, credential-safe one-at-a-time MCP secret collection and protected environment-file boundaries, and a bounded content-hashed standing-context snapshot that removes duplicate standing reads while preserving provider prompt-cache reuse. Post-turn Memory Router skipping remains an evaluated follow-up, not part of this release. Current npm launcher source is prepared as `@socrates-ai/cli@0.1.18` for manual npm publish. The launcher prefers direct GitHub Release asset URLs before falling back to REST metadata so rate limits do not block public `npx` installs.
