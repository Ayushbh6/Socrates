...
Conversation terminals are scoped by `projectId + conversationId + workspacePath`. Running Terminals are task-aware state, not fixed-age resources: a healthy Terminal referenced by a running or waiting Socrates task is not terminated after two hours. Cleanup remains explicit stop, conversation deletion, workspace invalidation, app shutdown, confirmed process loss, and deliberate completed-output retention. `wait` is a separate event-driven model tool. After independent work is exhausted, Socrates registers named Terminal dependencies with `completed`, `failed`, and/or `input_required`; the backend durably suspends the task and ends the model invocation without a final answer. The deterministic coordinator wakes a continuation of that same task only on a requested event, with bounded new output and exit/status evidence. There is no model-facing polling interval. `wait.reason` is required, no more than 7 words and 64 characters.
...
`skills` is a model-visible discovery/inspection tool for builtin, global, and project skills. The preferred model path is `skills({ operation: "list" })` with an optional `scope`, followed by `skills({ operation: "describe", id: "<exact-listed-id>" })` or an exact listed `name`. `list` returns compact rows with id/name/scope/description; `describe` returns the selected `SKILL.md` content. For ordered multi-step work, verification/review sequences, and closure/handoff requests, main Socrates must perform this list/describe discovery before domain tools; knowing the generic tools does not replace checking learned user-specific gates. The main Socrates agent cannot create, edit, or delete skills through this tool. Skill creation/update should route through explicit UI approval plus the Skill Writer Agent, whose narrow internal write path is `skill_write`.
