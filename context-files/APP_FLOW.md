...
Current runtime release is the published, non-prerelease GitHub Release `v0.1.18`, and the public npm registry serves `@socrates-ai/cli@0.1.18`. It includes the v0.1.17 provider, retrieval, model-catalog, Ollama, and runtime-packaging foundation plus semantic user-controlled memory opt-outs, complete persisted tool-call traces with distinct intent-discovery UI, credential-safe one-at-a-time MCP secret collection and protected environment-file boundaries, and a bounded content-hashed standing-context snapshot that removes duplicate standing reads while preserving provider prompt-cache reuse. Production continues to run the post-turn Memory Router at genuine finalization. A one-call semantic skip gate was evaluated and rejected because both 90-attempt runs produced unsafe skips; its synthetic dataset, opt-in script, and report are retained only as segmented evaluation evidence and are not imported by the app/runtime. The launcher prefers direct GitHub Release asset URLs before falling back to REST metadata so rate limits do not block public `npx` installs.
