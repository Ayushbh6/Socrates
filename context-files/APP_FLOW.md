...
`bash` is the compatibility id for the Terminal tool. It runs platform-native PTY commands from the active project workspace, while UI/product copy says Terminal. `operation: "list"` returns at most 12 compact named rows and must be used when the current Terminal inventory is unclear. `run` is a unified PTY path: a quick command returns normally, while any raw shell run still active after `SOCRATES_TERMINAL_AUTO_DETACH_MS` (default 15 seconds) becomes the same conversation-scoped Terminal without being killed or restarted. `start` remains the immediate-background option. `status`, `output`, and `stop` use human names only; opaque ids and cursors remain internal. Model-visible Terminal output is capped at 16,000 characters, list text at 12,000 characters, and durable full output remains in the UI/audit store.

Conversation terminals are scoped by `projectId + conversationId + workspacePath`. Running Terminals are task-aware state, not fixed-age resources: a healthy Terminal referenced by a running or waiting Socrates task is not terminated after two hours or merely because the main server restarts. Normal server close stops polling but leaves active sessions owned by the independent Terminal supervisor; startup reconnects, drains persisted output, and resumes monitoring. Cleanup remains explicit stop, conversation deletion, workspace invalidation, confirmed process loss, deliberate completed-output retention, or an explicit supervisor shutdown. `wait` is a separate event-driven model tool. After independent work is exhausted, Socrates registers named Terminal dependencies with `completed`, `failed`, and/or `input_required`; the backend durably suspends the task and ends the model invocation without a final answer. The deterministic coordinator wakes a continuation of that same task only on a requested event, with bounded new output and exit/status evidence. Wake claim and continuation claim are atomic. If the server stops after a continuation claim but before that continuation finishes, startup cancels the stale turn and requeues the same durable task for one new continuation attempt. There is no model-facing polling interval. `wait.reason` is required, no more than 7 words and 64 characters.

Each Socrates home owns a separate Terminal supervisor socket. The supervisor health handshake reports its instance, process, start time, and Terminal count. Ordinary Terminal operations never replace a supervisor merely because one request timed out: the client performs bounded reachability retries, and background polling requires three consecutive failures before declaring the session `missing`. Startup reconciliation records whether each active Terminal was reconnected, its process was missing from a healthy supervisor, or the supervisor itself was unavailable. Missing/uncontrollable sessions become a deterministic `failed` wake for any waiting task. This recovers task progress but does not claim transparent PTY survival when the supervisor process itself was killed.
...
Always-apply rules are a tiny curated layer, not a new memory store. Treat them as one centralized always-apply rules list with two lanes: `user_profile.md` has a capped `Global Always-Apply Rules` section, and workspace project memory has a capped `Project Always-Apply Rules` section. Each section is limited to 10 human-readable rules. The byte-stable prelude order is bounded identity core (`core_identity`, `voice_and_presence`, `relationship_to_user`), global rules, project rules, then the compact map generated from `packages/contracts/src/socratesSurfaces.ts`. It is inserted before conversation/user text even when structured Memory Router generation is unavailable. Short project rules may reference fuller repo doctrine, while user/project metadata, router results, runtime facts, retrieval, attachments, tools, and ledgers stay dynamic afterward.
...
`mcp_registry` is the model-visible MCP discovery and lifecycle tool. `list`, `describe`, and `check` are read/validation operations; `configure` and `delete` support project or global servers only after the normal approval UI. Chat configure accepts an exact user-supplied/trusted stdio command, stores secrets separately, saves disabled, performs the handshake/tool listing inside the approved operation, and enables only on success. The dashboard provides the same lifecycle through JSON/TOML import or manual setup, editing, persistent status/tool counts, config-path copy/open actions, and deletion. Global servers are inherited by projects. The system prompt carries only concise registry-first guidance; dynamic `mcp__...` tools appear only after describe/check/configure exposes them.
