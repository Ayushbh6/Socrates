...
Both views use the same projects, primary workspace, workspace `.socrates/`, global `~/.Socrates/`, Socrates tool loop and within-turn disposition policy, MCP/skills/provider settings, Memory Router implementation, context compressor, and global Memory Agent. V2 owns the Flow/goal router, capsule projection, runtime transport, and `v2_*` persistence. See `V2_FLOW_ARCHITECTURE.md` for the complete inheritance boundary and current limitations.
...
Project
  ...
  └── V2 Seamless Flow, feature flagged
        └── one persistent Flow
              ...
              ├── query-history outline
              ├── one foreground goal plus parked capsules
...
The Flow project view uses the same warm cream Socrates surface as Classic, but its center is a focused projection of one exchange rather than the whole chronological transcript. The current user request and Socrates response use the shared Classic transcript/activity components; older exchanges stay available through the sidebar query outline, and long user requests collapse behind **Show more**. The living sphere remains a faint background layer while the exchange is settled and takes visual prominence only while Socrates is actively listening, routing, thinking, working, or awaiting input. Two single lightweight notes float above that one canvas: **Live Context** and **Current Focus / Current Task**. They can be dragged only by their complete circular paperclip handles, moved precisely with the arrow keys, clamped into the responsive workspace, and persisted per project. The focus note always displays the backend-authoritative active goal rather than deriving a label from the visible query. Opening either note reveals the larger Context/Focuses/Activity inspector; that inspector can be pinned or dismissed, and keeps detailed evidence, focus lifecycle, approvals, tools, credentials, Terminals, and speech settings out of the calm default surface.

The outer shell is not a V2 imitation: both views render the shared `ProjectChatSidebar` shell and `WorkspaceTopbar`, so full collapse, hover bounds, dimensions, and dashboard treatment stay identical. The shell is fixed-width and viewport-height with a fixed heading and controls; only its active names list scrolls. Their sidebar content deliberately differs: Classic shows its nested project/conversation hierarchy, while Flow opens on a Queries-only level and uses a small back control to switch to a separate Projects-only level. Choosing a project returns to its Queries level. Projects and queries never share one long scroll surface, and Flow never invents Flow-side conversations or New Chat controls. Selecting an older query changes only the displayed exchange, not the active goal; **Return to current** restores the live exchange. Sending from that viewing state also returns to current and appends the new exchange after the live Flow tail, never as a branch from history. In Flow the shared sidebar is an overlay drawer: opening it covers the canvas instead of shifting, shrinking, or reflowing the notes and composer. **Classic View** opens or reuses the active goal's explicit preferred Classic home. Flow renders the same assistant Markdown/GFM, activity/tool rows, approvals, credentials, Terminal dock, message attachments, and final response treatment as Classic.
