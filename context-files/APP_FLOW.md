...
Terminal lifecycle state is monotonic across PTY output, input, polling, WebSocket delivery, and reload hydration. Each real lifecycle transition increments `stateVersion`; input commits `running` before its bytes reach the PTY, prompt detection is generation-bound, unchanged polls emit no duplicate status event, and the frontend rejects snapshots older than the version it already holds. This prevents a delayed `running` event from visually overwriting a newer second `awaiting_input` prompt.
