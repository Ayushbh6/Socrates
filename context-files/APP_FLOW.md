...
Release packaging is validated against `pnpm@11.7.0`. GitHub Actions runs pnpm 11 under Node 24, while the produced runtime archive still bundles Node v20.20.2. The server runtime deploy step uses pnpm's legacy deploy implementation because the workspace does not use injected workspace packages. The workspace explicitly allows build scripts only for the packages that need them: native runtime dependencies `better-sqlite3` and `@homebridge/node-pty-prebuilt-multiarch`, plus dev packaging toolchain packages `esbuild` and `unrs-resolver`.
