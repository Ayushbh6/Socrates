...
The `read`, `search`, `trace_retrieve`, `tool_docs`, `skills`, `soul`, `user_profile`, and `list_project_resources` tools are read-only. `mcp_registry` list/describe/check are automatic discovery/validation, while configure/delete are mutation-lane operations that require approval and an exact user-supplied or trusted stdio config; never invent packages, URLs, or credentials. `skills` model calls remain limited to list/describe. `project_docs` and `repo_docs` retain their scoped mutation boundaries and required preflight before approval-required mutations.
