...
- Do not add V2 goal capsules, V2 context-projection state, Flow runtime events, V2 speech jobs/artifacts, read-aloud, or V2 voice orchestration to the V1 chat path. Shared Socrates within-turn tool-output disposition is core execution policy and therefore applies identically in Classic and Flow, but it must not create V2 rows in Classic. The other approved shared seams are narrow: Classic pre/post Memory Router phases may select/finalize canonical project goals through compact metadata, and the Classic microphone may call shared transcriber adapters through the conversation-scoped temporary-STT route and append text to the unsent draft. Neither seam may duplicate V2 execution state, auto-send, or create V2 speech/runtime records.
- Do not migrate or reinterpret existing V1 conversations as V2 Flows.
- Do not create hidden V1 conversation rows as foreign-key shims or put V2 ids into V1 conversation fields. V2-owned bridge/link rows are the only cross-view persistence: a Classic conversation may contain many canonical goals, every routed Classic turn links to exactly one goal, and each goal may have at most one preferred Classic home. A Classic home is created only when the user explicitly opens that goal in Classic. Visible Q&A may mirror idempotently, while tools, evidence, usage, events, and orchestration remain owned by their source runtime.
...
- Keep goal-aware Flow projection, V2 runtime events, and all conversation-owned execution persistence in the V2 path. Those layers may choose the active goal, attach its capsule, and store namespaced exact audit rows, but they must not automatically inject prior-turn tool evidence into the next model request. Visible Q&A, the goal capsule, and shared 170k compaction are the normal continuation context; exact older evidence is retrieved explicitly when needed. The canonical goal ledger and exact Classic turn-goal links are the bounded shared navigation seam; shared memory-note or Memory Agent work must retain exact source coordinates without appending events to the other runtime.
