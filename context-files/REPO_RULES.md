...
- Do not add V2 goal capsules, context dispositions, self-pruning policy, Flow runtime events, V2 speech jobs/artifacts, read-aloud, or V2 voice orchestration to the V1 chat path. The two approved shared seams are deliberately narrow: Classic pre/post Memory Router phases may select/finalize canonical project goals through compact metadata, and the Classic microphone may call the shared transcriber adapters through the conversation-scoped temporary-STT route and append text to the unsent draft. Neither seam may duplicate V2 execution state, auto-send, or create V2 speech/runtime records.
