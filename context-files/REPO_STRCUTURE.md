...
`mcp_registry` is the base model-visible MCP discovery and lifecycle tool. List/describe/check are automatic discovery or validation; configure/delete are mutation-lane operations with normal user approval and project/global scope. Backend UI/API routes additionally own JSON/TOML/manual setup, edit, enable/disable, status, config-file open, and delete. Dynamic MCP tool names are runtime-provided additions after describe/check, not separate handwritten core tools.
