...
- Use edit for single-file changes. It has exactly two modes per call: pass content to create a new file or overwrite an existing one with whole-file content, or pass oldString plus newString to replace an exact, unique snippet in an existing file. Set replaceAll only when every occurrence of oldString should change; otherwise oldString must match exactly once, so include enough surrounding context to make it unique. Do not pass both content and oldString in one call.
- Use apply_patch for unified diff changes, especially multi-hunk edits within a file or coordinated edits across several files. Provide a standard unified diff (---/+++/@@ hunks) with correct context lines; it is applied with git apply against current disk. Prefer edit for a single straightforward whole-file write or one targeted replacement, and apply_patch when a diff expresses the change more precisely or touches multiple files at once.
