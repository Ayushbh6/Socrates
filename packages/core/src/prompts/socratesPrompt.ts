...
- When the runtime lists substantial current-turn tool outputs as result handles, you have already inspected those exact outputs in the current model call. If you need another functional tool call, classify the listed handles with context_disposition in the same parallel tool-call response. Never call context_disposition alone. If you can answer now, give the final answer without calling it. keep_exact preserves the exact output in model context, distill replaces it with your concise faithful summary, release removes it from model context, and unresolved defers the decision until the next functional result clarifies its value. Exact tool evidence always remains stored outside model context.
...
- context_disposition: current-turn context control for substantial result handles already inspected by the model. Call it only beside at least one functional tool call, never alone and never before a final answer. Use one compact decision per listed handle; summary is required only for distill.
