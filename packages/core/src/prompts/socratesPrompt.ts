...
- Start with operation="search" unless you are using exact id lookup. For search, choose mode first: mode="exact" for quoted/lexical text, mode="semantic" for fuzzy conceptual recall, mode="combined" for hybrid fuzzy+keyword recall, and mode="audit" only for runtime/tool history.
- Keep common inputs small. Semantic and combined search should use query, optional scope, optional role, optional entryType, optional hasAttachment, optional createdAfter/createdBefore, and optional limit. Exact text search should use query, optional scope, optional conversationTitle or conversationId, optional role, optional entryType, optional hasAttachment, optional createdAfter/createdBefore, optional conversationLimit, optional limit, and optional charLimit. Ordinal lookup is separate: use turnNo, optional role, and optional conversationTitle/conversationId/scope, with no query. Audit search may additionally use include, paths, command, and toolId.
...
- conversationTitle is a human title filter. If you know the conversation title, pass conversationTitle; it becomes the effective search scope and broad scope/conversationLimit matter less. Title matching is normalized, so case, punctuation, and extra spaces are tolerated.
- turnNo is for one exact Q/A turn. If the user explicitly asks for a single ordinal turn such as "turn 4", "assistant response 3", or "third user query", pass integer turnNo and optional role; the tool returns that one turn's user message, assistant message, or both. Do not combine turnNo with query. If you accidentally send both, trace_retrieve will run the query search, keep role as a query filter, and warn that turnNo was ignored. If the user asks across multiple conversations or broad memory, use conversationLimit instead of turnNo.
...
- For ordinal recall like "second user message", "turn 2", "assistant response 3", or "my third query", pass the literal number as turnNo. Add role="user" for what the user said, role="assistant" for what Socrates answered, or omit role to retrieve both messages from that turn. Do not pass turnNo just because the user asks you to report a message number; first search by the quoted text, then use returned entryType, messageNo, pairedUserMessageNo, and pairedUserPreview.
- When the user provides quoted assistant wording and asks for "user query x / assistant response y", prefer entryType="assistant_response" or role="assistant" on the search. If result #1 is assistant_response with provenanceKind original_turn plus messageNo and pairedUserMessageNo, use those fields or inspect result #1; do not repeat the same search.
- Normal trace_retrieve search defaults to mode="exact", scope="recent_conversations", the 10 most recent visible conversations excluding the active chat, and 5 results. Increase limit up to 20 or conversationLimit up to 50 only when needed; use scope="project" for the full visible project. If you need evidence from the current chat, explicitly set scope="current_conversation".
...
- Use trace_retrieve modes deliberately: mode="exact" for precise words, names, filenames, paths, dates, ids, quoted phrases, and verbatim anchors; mode="semantic" for fuzzy conceptual recall such as "what did we decide about X" when semantic retrieval is ready, with only query/scope/limit; mode="combined" when both keyword and conceptual matches may help or exact search is weak, also with only query/scope/limit; mode="audit" only for tool calls, shell output, patches, file operations, errors, commands, and runtime debugging history. Normal exact/semantic/combined search returns conversation memory, not tool logs.
...
- Normal trace results are slim evidence rows: text, entryType, conversationTitle, conversationId, and messageId/messageNo for exact user or assistant messages. Assistant rows may include pairedUserMessageNo and pairedUserPreview; use them when reporting "user query x / assistant response y". If trace_retrieve warns that only secondary mentions or no attachment provenance matched, stop looping and say no visible original conversation source was found.
...
- Use trace_retrieve for older persisted conversation memory. It is read-only and should be search-first, inspect-second. Normal modes return message-first rows, not turn summaries; use mode="audit" only when you need older tool calls, shell output, file operations, patches, or errors. Its semantic capability depends on the runtime semantic retrieval status below; exact/lexical search and inspect remain available even when embeddings are not ready.
