...
export const memoryNoteTool: SocratesTool<typeof memoryNoteToolInputSchema._type, typeof memoryNoteToolOutputSchema._type> = {
  name: "memory_note",
  description:
    "Send one short notepad lead to the Global Memory Agent only when the current turn contains an important durable memory candidate. Input is only note and optional importance. Do not decide the memory target, skill scope, skill name, or action. The backend attaches the current user message and trace refs automatically.",
  ...
  summary: (output) => `Created memory note #${output.noteNumber}.`,
  resultPreview: (output) => `memory note #${output.noteNumber} opened; attachedSource=${output.attachedSource}`,
}
