...
export const repoDocsTool: SocratesTool<typeof repoDocsToolInputSchema._type, typeof repoDocsToolOutputSchema._type> = {
  name: "repo_docs",
  description:
    "Read, search, or patch the active workspace's .socrates/repo_docs/*.md doctrine files. Patch is a constrained oldText/newText replacement against one allowlisted repo-doc file only; use it for durable repo behavior, contracts, workflows, and pitfalls.",
  ...
  metrics: (output) => ({ filesRead: output.operation === "patch" ? 0 : (output.paths?.length ?? 1), filesEdited: output.changed ? 1 : 0 }),
}
