...
export const traceRetrieveTool: SocratesTool<typeof traceRetrieveToolInputSchema._type, typeof traceRetrieveToolOutputSchema._type> = {
  name: "trace_retrieve",
  description:
    "Investigate older visible project conversation memory. Default operation is search. With no query and mode omitted/exact, browse Q/A windows by recent/project/title/id/date/offset using conversationLimit, conversationOffset, perConversationLimit, role, entryType, hasAttachment, updatedAfter/updatedBefore, createdAfter/createdBefore, limit, and charLimit; Q/A pairs count as one result unless role is specified. With query, exact/semantic/combined search returns message-first evidence rows with broad snippets around the best match. Use exact for quoted text, filenames, paths, names, ids, dates, and lexical terms; semantic or combined for fuzzy/hybrid recall and always provide query. scope=\"project\" means all visible active/archived project conversations unless narrowed; scope=\"recent_conversations\" excludes the active chat by default. For latest/previous/recent conversation questions, browse first, then inspect/search precise results. For quoted assistant wording where the user asks for user query x / assistant response y, prefer entryType=\"assistant_response\" or role=\"assistant\" and use returned messageNo/pairedUserMessageNo. Use audit only for tool calls, shell output, file operations, patches, errors, commands, or runtime history. turnNo is exclusive exact ordinal lookup: use it only for explicit turn 4/assistant response 3 style requests, do not combine with query. If query and turnNo are both provided, query search runs, role is kept as a query filter, and the output warns that turnNo was ignored.",
