...
Current npm runtime release target is `v0.1.7`. It includes the v0.1.6 usage/cost accounting, OpenRouter provider-routing/cache-affinity fixes, plus the direct-provider pricing snapshot fix for Gemini 3.1 Pro and Gemini 3.1 Flash-Lite. The dedicated `memory-work-v1` branch remains separate from this release target.
