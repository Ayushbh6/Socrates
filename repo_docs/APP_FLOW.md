...
Current runtime release target is GitHub Release `v0.1.9`. It includes the v0.1.8 provider/cache/runtime fixes plus the compressor refactor, memory-agent packing fix, proactive investigation harness, repo-docs preflight gate, and durable project-memory checkpoint. Current npm launcher target is `@socrates-ai/cli@0.1.10`, a launcher-only fix that prefers direct GitHub Release asset URLs before falling back to REST metadata so rate limits do not block public `npx` installs.
