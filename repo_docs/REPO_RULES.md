...
The `edit` tool is the primary V1 model-visible single-file mutation tool. Existing files should use targeted `oldString`/`newString` replacements; whole-file `content` on an existing file requires explicit `overwrite: true` and should be reserved for deliberate rewrites. The separate `apply_patch` tool covers multi-hunk or multi-file unified diff application via git apply. Both must show a diff or equivalent preview and require approval unless the user explicitly runs a full-access mode. The harness tracks file freshness from `read` results; model-facing tool inputs must not carry content hashes.
