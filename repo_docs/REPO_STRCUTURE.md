...
`mcp_registry` is the base model-visible MCP management tool. It lists, describes, checks, and configures supported no-secret MCP presets such as Playwright. Dynamic MCP tool names are runtime-provided additions, not separate handwritten core tools.
