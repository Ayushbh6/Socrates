...
The `read` and `edit` implementations together own file freshness. `read` returns full-file content hashes and file metadata. `edit` remains the only model-visible file mutation tool, but internally it must read/stat/hash before non-dry-run writes, require base hashes for overwrites and patches against existing files, write through a safe same-directory temp file, and verify the final bytes on disk before reporting success. Failed freshness or read-back checks are recoverable workspace errors, not successful edits.
